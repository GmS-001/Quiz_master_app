<template>
    <div class="quiz-background" @copy.prevent @paste.prevent @cut.prevent>
      <div class="container py-4">
        <div v-if="isLoading" class="text-center text-white"><h2>Loading Quiz...</h2></div>
            ...
            <div class="col-md-9">
              <div class="quiz-header text-white mb-4 d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center">
                  ...
                  <i class="bi bi-info-circle-fill fs-4 me-3" style="cursor: pointer;" data-bs-toggle="modal" data-bs-target="#rulesModal"></i>
              ...
              <div class="card">
                <div class="card-header d-flex justify-content-between">
                  ...
                  <button class="btn btn-sm" :class="questionStates[currentQuestion.id] === 'review' ? 'btn-warning' : 'btn-outline-warning'" @click="toggleReview">
                    <i class="bi bi-flag-fill me-1"></i> {{ questionStates[currentQuestion.id] === 'review' ? 'Unmark' : 'Mark for Review' }}
                  </button>
                </div>
                <div class="card-body">
                  ...
                  <div class="list-group">
                    <label v-for="i in 4" :key="i" class="list-group-item list-group-item-action">
                      <input class="form-check-input me-2" type="radio" :name="'q' + currentQuestion.id" :value="i" v-model="userAnswers[currentQuestion.id]" @change="markAsAnswered">
              ...
              <div class="d-flex justify-content-between mt-4">
                <button @click="prevQuestion" class="btn btn-light" :disabled="currentQuestionIndex === 0">Previous</button>
                <button @click="nextQuestion" class="btn btn-light" :disabled="currentQuestionIndex === questions.length - 1">Next</button>
              </div>
            </div>
              ...
              <div class="card">
                ...
                <div class="card-body question-palette">
                  <button v-for="(question, index) in questions" :key="question.id" 
                          ...
                          class="btn m-1"
                          :class="getPaletteClass(question.id)">
                    {{ index + 1 }}
                  </button>
                </div>
      ...
      <div class="modal fade" id="rulesModal" tabindex="-1">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Quiz Rules</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" @click="closeRulesModal"></button></div>
  ...
  <style scoped>
  .quiz-background { background: linear-gradient(45deg, #4c68d7, #6a349d); min-height: 100vh; width: 100vw; position: fixed; top: 0; left: 0; overflow-y: auto; user-select: none; /* Disables text selection */ }
